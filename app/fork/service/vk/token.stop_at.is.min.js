...
var createNotifyEmail = function(uid, msg, cb) {
						...
						(function(item){
							...
							async_arr.push(function(callback){
								...
								if(item.stop_at < _now) {
									...
									createNotifyEmail('service.vk.token.' + item.profile + '.' + item.token_id + '.stopped', {
										email : item.profile_email,
										subject : 'Окончен оплаченный период работы аккаунта',
										tpl : azbn.mdl('path').normalize(__dirname + '/../../email/tpl/service/vk/token.stopped.html'),
									}, function(){
										callback(null, null);
									});
									...
									createNotifyEmail('service.vk.token.' + item.profile + '.' + item.token_id + '.veryhot', {
										email : item.profile_email,
										subject : 'Всего 2 дня до окончания оплаченного периода',
										tpl :  azbn.mdl('path').normalize(__dirname + '/../../email/tpl/service/vk/token.veryhot.html'),
									}, function(){
										callback(null, null);
									});
									...
									createNotifyEmail('service.vk.token.' + item.profile + '.' + item.token_id + '.hot', {
										email : item.profile_email,
										subject : 'Менее недели до окончания оплаченного периода',
										tpl :  azbn.mdl('path').normalize(__dirname + '/../../email/tpl/service/vk/token.hot.html'),
									}, function(){
										callback(null, null);
									});
